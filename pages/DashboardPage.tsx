...
import { Activity, ShieldCheck, AlertOctagon, Plus, BrainCircuit } from 'lucide-react';
...
export const DashboardPage: React.FC = () => {
    ...
    const createRandomTransaction = (safe: boolean = false): Transaction => {
        const isAnomaly = !safe && Math.random() > 0.7;
    ...
    const analyzeTransaction = async (tx: Transaction) => {
        ...
        if (selectedTransaction?.id === tx.id) {
            setSelectedTransaction(prev => prev ? { ...prev, ...analysis, status: 'cleared' } : null);
        }
    };
    ...
    return (
        <div className="p-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-full min-h-[500px] mt-8">
